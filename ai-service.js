...
async function generateNewsDigest(config = {}, userLanguage = 'ru', broadcastPrompt = '') {
  ...
  const brandBlock =
    `${theme ? `Общая тематика и угол бренда из панели (дополняет рассылку, не заменяет основной запрос ниже, если он задан): ${theme}` : 'Общая тематика из панели не задана — опирайся на основной запрос рассылки или на краткий обзор.'}
${role ? `Роль и тон (соблюдай): ${role}.` : ''}
${rules ? `Правила оформления и стиля (соблюдай):\n${rules}` : ''}`.trim();

  const systemPrompt = prompt
    ? `Ты бот рассылок WhatsApp. Подготовь один готовый пост на ${langName} языке.

ГЛАВНОЕ — СОДЕРЖАНИЕ И ТЕМА ЭТОГО СООБЩЕНИЯ (выполняй в первую очередь, это то, что пользователь задал в интерфейсе рассылки):
${prompt}

Настройки бренда из панели (тон, рамки, вторичный фокус — не перечёркивают главный запрос выше):
${brandBlock}

База знаний (единственный источник фактов, если в тексте нужны проверяемые данные):
${BROADCAST_KNOWLEDGE_BASE}

КРИТИЧЕСКИЕ ПРАВИЛА:
1) Тема, структура и посыл сообщения должны следовать блоку «ГЛАВНОЕ» выше.
2) Тематика/роль/правила из панели — для стиля и контекста; если конфликт с главным запросом, побеждает главный запрос.
3) Используй только факты из базы знаний и из явных формулировок промпта; не выдумывай цифры, имена, даты, цены.
4) Если факта нет в базе — формулируй нейтрально, без неподтверждённых деталей.

Требования: один блок текста для чата; без заголовков с символом #; эмодзи умеренно; длина примерно 400–2000 символов.`
    : `Ты бот рассылок WhatsApp. Подготовь один готовый пост на ${langName} языке.
...
КРИТИЧЕСКИЕ ПРАВИЛА:
1) Используй только факты из базы знаний.
2) Не выдумывай данные, проценты, имена, даты, цены и результаты.
3) Если факта нет в базе, не фантазируй — дай нейтральную формулировку без неподтверждённых деталей.
...
Требования: один блок текста для отправки в чат; без заголовков с символом #; эмодзи — умеренно; длина примерно 400–2000 символов; не выдумывай конкретные даты событий и цифры, если не уверен — формулируй осторожно.`;
  ...
  const userDigestAsk = prompt
    ? userLanguage === 'en'
      ? `Write one broadcast message whose topic and content follow the MAIN block in the system prompt first. Use only verified facts from the knowledge base where facts are needed.`
      : `Напиши одно сообщение-рассылку: по смыслу и теме в первую очередь следуй блоку «ГЛАВНОЕ» в системном промпте. Где нужны факты — только из базы знаний.`
    : theme || role || rules
      ? userLanguage === 'en'
        ? `Write one broadcast message. Follow the panel theme and rules; use only verified facts from the knowledge base.`
        : `Напиши одно сообщение-рассылку по тематике и правилам из панели; факты — только из базы знаний.`
      : userLanguage === 'en'
